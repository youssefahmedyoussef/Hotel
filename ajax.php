...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($connection, $sql)) >= 1) {
            $response['done'] = false;
            $response['data'] = "الغرفة موجودة  بالفعل";
        } else {
            ...
            if ($result) {
                $response['done'] = true;
                $response['data'] = 'تم الاضافة بنجاح';
            } else {
                $response['done'] = false;
...
if (isset($_POST['edit_room'])) {
    ...
    if ($room_no != '') {
        ...
        if ($result) {
            $response['done'] = true;
            $response['data'] = 'نجح تحرير الغرفة';
        } else {
            $response['done'] = false;
            $response['data'] = "خطا";
        }
        ...
        $response['done'] = false;
        $response['data'] = "الرجاء إدخال رقم الغرفة";
    }
...
if (isset($_POST['room_type'])) {
    ...
    $result = mysqli_query($connection, $sql);
    if ($result) {
        echo "<option selected disabled>حدد نوع الغرفة</option>";
        while ($room = mysqli_fetch_assoc($result)) {
            echo "<option value='" . $room['room_id'] . "'>" . $room['room_no'] . "</option>";
        }
    } else {
        echo "<option>No متوفرة</option>";
    }
}
    ...
    if ($customer_result) {
        ...
        if ($booking_result) {
            ...
            if (mysqli_query($connection, $room_stats_sql)) {
                $response['done'] = true;
                $response['data'] = 'تم الحجز بنجاح';
            } else {
                $response['done'] = false;
                $response['data'] = "خطأ في قاعدة البيانات ";
            }
        } else {
            $response['done'] = false;
            $response['data'] = "حجز خطأ في قاعدة البيانات";
        }
    } else {
        $response['done'] = false;
        $response['data'] = "خطأ في قاعدة البيانات في إضافة العميل";
    }
    ...
    if ($booking_id != '') {
        ...
        if ($result) {
            ...
            if ($updateResult) {
                ...
                $response['done'] = false;
                $response['data'] = "مشكلة في غرفة التحديث تحقق في الحالة";
            }
        } else {
            $response['done'] = false;
            $response['data'] = "مشكلة في الدفع";
        }
    } else {
        $response['done'] = false;
        $response['data'] = "خطا";
...
if (isset($_POST['check_out_room'])) {
    ...
    if ($booking_id != '') {
        ...
        if ($remaining_price == $remaining_amount) {
            ...
            $response['done'] = false;
            $response['data'] = "الرجاء دفع المبلغ كامل حتى تستطيع انهاء الحجز";
        }
    } else {
        $response['done'] = false;
        $response['data'] = "خطا في الحجز";
    }
    echo json_encode($response);
}
